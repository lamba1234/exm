<?php  
  class Expense extends Base {
    ...
    // Handles file upload for expenses
    public function uploadExpenseFile($UserId, $expenseId, $file) {
      if (!isset($file['tmp_name']) || !is_uploaded_file($file['tmp_name'])) {
        return false;
      }

      // Validate file type (only allow common document and image formats)
      ...
      $newFilename = uniqid() . '_' . $UserId . '_' . $expenseId . '.' . $fileExtension;
      ...
      $stmt = $this->pdo->prepare("UPDATE expense SET file_path = :file_path WHERE ID = :expense_id AND UserId = :user_id");
